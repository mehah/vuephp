...
/*
 * ! Vue.js v2.5.16 (c) 2014-2018 Evan You Released under the MIT License.
 */
...
Vue.processApp = function(appName, template, jsonData, methodsList, $appJs) {	
	var el = null;
	if(!(appName in Vue.options.components)) {		
		...
		if($appJs) {
			$appJs.call(component, function(id, component) {el = id;});
		}
	...
	if(el) {
		if(el in Vue.contexts) {
			...
			setTimeout(function() {
				Vue.util.merge(context.$refs[appName], jsonData);
			});
		} else {
			data.currentView = appName;		
			Vue.contexts[el] = new Vue({
	    		el : el,
	    		data: data,
	    		mixins: [{data: Vue.globalData}]
	    	});		
		}
	}
}
...
Vue.util.merge = function(o1, o2) {
	if(!o2 || Object.prototype.toString.call(o2) !== '[object Object]') {
		return;
	}
	
	for(var i2 in o2) {
		if(i2 === '__ob__') { continue; }
