/*
 * jrender-crossbrowser
 * */
Object.keys||(Object.keys=function(){"use strict";var t=Object.prototype.hasOwnProperty,e=!{toString:null}.propertyIsEnumerable("toString"),n=["toString","toLocaleString","valueOf","hasOwnProperty","isPrototypeOf","propertyIsEnumerable","constructor"],r=n.length;return function(o){if("object"!=typeof o&&("function"!=typeof o||null===o))throw new TypeError("Object.keys called on non-object");var l,i,p=[];for(l in o)t.call(o,l)&&p.push(l);if(e)for(i=0;i<r;i++)t.call(o,n[i])&&p.push(n[i]);return p}}()),"remove"in Element.prototype||(Element.prototype.remove=function(){this.parentNode&&this.parentNode.removeChild(this)}),Element.prototype.content=function(){return this.contentDocument||this.contentWindow.document},"querySelectorAll"in Element.prototype||(JRender.util.loadScript(JRender.CONTEXT_PATH+"/jscript/jrender/sizzle.js",!1),Element.prototype.querySelectorAll=function(t){return Sizzle(t,this)},Element.prototype.querySelector=function(t){return Sizzle(t+":first",this)[0]||null}),"getElementsByClassName"in Element.prototype||(Element.prototype.getElementsByClassName=function(t){return this.querySelectorAll("."+t)}),"hasAttribute"in Element.prototype||(Element.prototype.hasAttribute=function(t){return void 0!==this[t]});
...
/*
 * ! vue-resource v1.5.0 https://github.com/pagekit/vue-resource Released under
 * the MIT License.
 */
...
var VUE_GLOBAL = {
	...
	methods: {
		...
		redirect: function(url, param) {
			...
			if(param) {
				if(typeof param === 'object') {
					param = {arg0: JSON.stringify([param])};
				} else {
					url += '/'+param;
					param = {};
				}
			...
			param.cached = appName in Vue.pageCache; 
			
			this.$http.post(url, param, {emulateJSON: true}).then(function(data) {
				data = data.body;
				updateContent(this.$el, data, url);
		    });
		},
		request: function(url) {
			...
			this.$http.post(url, {arg0: JSON.stringify(params)}, {emulateJSON: true}).then(function(data) {
				data = data.body;
				if(data) {
					if(data.d) {
						objectAssignDeep(this, data.d);
					}
...
window.onpopstate = function(event) {
	Vue.http.post(event.target.location.pathname, {cached: true}, {emulateJSON: true}).then(function(data) {
		eval(data.body);
    });	  
};
...
function processAppTemplate(appName, CURRENT_TEMPLATE_CONTENT, jsonData, methodsList, $appJs, $executeMethods) {
	var app = Vue.pageCache[appName];
	if(!app) {
		app = {
			context: null,
			name: appName,
			template: CURRENT_TEMPLATE_CONTENT,
			init: $appJs,
			methods: methodsList,
			onCreate: $executeMethods
		}
	}
	
	var TEMP_OBJECT = {data: jsonData, methods: app.methods};
	if(app.init) {
		app.init.call(TEMP_OBJECT);
	}

	if(!TEMP_OBJECT.el) {
		throw "Element not defined in {appName}.js";
	}		
    ...
    if(elementPrincipal){
        ...
        var content = elementPrincipal.querySelector("app-content");
        if(content){
            content.innerHTML = app.template;

            app.context = new Vue({el : TEMP_OBJECT.el, mixins: [VUE_GLOBAL, TEMP_OBJECT], created: app.onCreate});
        }
    }
}

Vue.util.clear = function(o) {
	for(var i in o) {
		if(i === '__ob__') {
			continue;
		}
...
function updateContent(el, script, url) {
	if(typeof el === 'string') {
		el = document.getElementById(el);
	}
	
	el.querySelector('app-content').innerHTML = '';
...
Vue.directive('goto', {
  ...
  unbind: function (el, binding, vnode) {
    this.el.removeEventListener('click', el.handler)
  }
});