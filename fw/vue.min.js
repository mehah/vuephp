...
window.onpopstate = function(event) {
	Vue.http.post(event.target.location.pathname, null, {emulateJSON: true}).then(function(data) {
		eval(data.body);
    });	  
};

function processAppTemplate(CURRENT_TEMPLATE_CONTENT, jsonData, methodsList, $appJs, $executeMethods) {
	var TEMP_OBJECT = {data: jsonData, methods: methodsList};
	if($appJs) {
		$appJs.call(TEMP_OBJECT);
	}
    ...
    if(elementPrincipal){
        ...
        if(content){
            content.innerHTML = CURRENT_TEMPLATE_CONTENT;

			_VUE = VUE_CONTEXT[TEMP_OBJECT.el] = new Vue({el : TEMP_OBJECT.el, mixins: [VUE_GLOBAL, TEMP_OBJECT], created: function(){$executeMethods.call(this)}});
            _VUE.html = html;
        }
    }
}
